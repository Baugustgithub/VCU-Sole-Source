...
function handleReasonOptionSelection(element) {
  ...
  if (checkbox.checked) {
    ...
    formData.alternatives_reason_options = formData.alternatives_reason_options.filter(v => v !== value);
  }
}
      ...
      <div class="space-y-2 mb-4">
        ${valid.map(reason => `
          <div class="form-check" data-value="${reason}" onclick="handleReasonOptionSelection(this)">
            <label class="flex items-start"><input type="checkbox"><span class="ml-3">${reason}</span></label>
          </div>`).join('')}
      </div>
      <p class="text-sm text-red-700 font-medium mb-2">✘ Not Valid Justifications</p>
      <div class="space-y-2 mb-4">
...
function handlePriceSelection(element) {
  ...
  if (checkbox.checked) {
    ...
    formData.price_reasonable = formData.price_reasonable.filter(v => v !== value);
  }
...
function createStepFiveContent() {
  ...
  stepContent.innerHTML = `<p class="mb-4 text-gray-700">Select methods used to determine price reasonableness:</p>
    <div class="space-y-3">
      ${methods.map(method => `
...
function evaluateResult() {
  ...
  if (formData.single_source === "no") score -= 3;
  if (formData.single_source === "unsure") score -= 1;
  ...
  else if (formData.alternatives_reason_options.length) {
    const validAlt = formData.alternatives_reason_options.filter(reason =>
      reason.startsWith("The complexity") || reason.startsWith("A subject") || reason.startsWith("The product") ||
      reason.startsWith("Regulatory") || reason.startsWith("The technology")
    ).length;

    const invalidAlt = formData.alternatives_reason_options.length - validAlt;

    score += validAlt ? 1 : 0;
    score -= invalidAlt ? 1 : 0;
  }
